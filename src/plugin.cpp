...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"tempMute";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "tempMute";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return "Temporary mute microphone for example 5 seconds, then it will automatically unmute itself";
}
...
void ts3plugin_onHotkeyEvent(const char* keyword) {
	if (!strcmp(keyword, "mute")) {
		std::thread([] {
			ts3Functions.setClientSelfVariableAsInt(ts3Functions.getCurrentServerConnectionHandlerID(), CLIENT_INPUT_MUTED, INPUT_DEACTIVATED);
			ts3Functions.flushClientSelfUpdates(ts3Functions.getCurrentServerConnectionHandlerID(), NULL);
			std::this_thread::sleep_for(std::chrono::seconds(how_many_seconds));
			ts3Functions.setClientSelfVariableAsInt(ts3Functions.getCurrentServerConnectionHandlerID(), CLIENT_INPUT_MUTED, INPUT_ACTIVE);
			ts3Functions.flushClientSelfUpdates(ts3Functions.getCurrentServerConnectionHandlerID(), NULL);
		}).detach();
	}
}
